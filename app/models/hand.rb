...
def flush(set)
 ...
 return 7.0 + high_card(set).to_i / 100.0
end
    ...
    def straight(set)
     ...
     if(first == 1 && set[1].number.to_i == 10)
      ...
      if(i == 4)
       return 6 + high_card(set).to_i / 100.0 + set[3].number.to_i / 100.0
     end
...
return 6 + high_card(set).to_i / 100.0
end
...
def three_kind(set)
 ...
 uniq = set.uniq {|e| e.number}
  if(uniq.size == 4)
    head = uniq[0].number.to_i
    if(head != uniq[1].number.to_i)
      return 0;
    end
     if(head == 1 )
      head = 14
...
def two_pair(set)
  ...
  if(i== 2 && k == 2)
    ...
    if(first > second)
      return 4 + first / 100.0 + more / 100.0
    end

    return 4 + second / 100.0 + more /100.0
